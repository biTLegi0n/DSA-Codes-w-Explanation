 ...
 *        element and marking it as negative, i mean if arr[i] exists, then mark arr[arr[i]] as negative. This way, we can mark
 *        all the numbers in range [1,n] existing in the array and indices of only those which does not exists in the array remains.
 *
